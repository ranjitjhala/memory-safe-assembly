...
fn sha256_final_unsafe_asm(out: &mut [u8], ctx: &mut SHA256_CTX) -> Result<(), ()> {
    ...
    if n > (SHA256_CBLOCK - 8) {
        ms_memset(&mut ctx.data[n..], 0, SHA256_CBLOCK - n);
        n = 0;
        sha256_block_data_order_unsafe_asm(ctx.h.as_mut_ptr(), ctx.data.as_ptr(), 64);
    }
    ms_memset(&mut ctx.data[n..], 0, SHA256_CBLOCK - 8 - n);
    // Append a 64-bit length to the block and process it.
    // is big endian = true
...
fn sha256_unsafe_asm(data: &[u8], len: usize, out: &mut [u8]) {
    ...
    sha256_update_unsafe_asm(&mut ctx, data, len).expect("Update");
    sha256_final_unsafe_asm(out, &mut ctx).expect("Final");
}

pub fn sha256_digest_unsafe(msg: &[u8], output: &mut [u8]) {
    sha256_unsafe_asm(msg, msg.len(), output);
}
